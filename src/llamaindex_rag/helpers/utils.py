...
def setup_llm(provider, model, api_key, **kwargs):
    ...
    if provider == "cohere":
        ...
        raise ValueError(
            f"Invalid provider: {provider}. Pick one of 'cohere', 'openai', or 'mistral'."
        )
...
def setup_embed_model(provider, **kwargs):
    ...
    if provider == "cohere":
        Settings.embed_model = CohereEmbedding(
            model_name="embed-english-v3.0", **kwargs
        )
    elif provider == "openai":
        ...
        Settings.embed_model = FastEmbedEmbedding(
            model_name="BAAI/bge-base-en-v1.5", **kwargs
        )
    else:
        raise ValueError(
            f"Invalid provider: {provider}. Pick one of 'cohere', 'fastembed', or 'openai'."
        )


def setup_vector_store(
    qdrant_url, qdrant_api_key, collection_name, enable_hybrid=False
):
    """
...
def create_index(from_where, embed_model=Settings.embed_model, **kwargs):
    ...
    **kwargs: Additional keyword arguments for configuring the index, such as:
    ...
    if from_where == "vector_store":
        ...
        raise ValueError(
            f"Invalid option: {from_where}. Pick one of 'vector_store', or 'docs'."
        )
...
def ingest(transformations, documents, **kwargs):
    ...
    pipeline = IngestionPipeline(transformations=transformations, **kwargs)
...
def create_query_pipeline(chain, verbose=True):
    ...
    """
    pipeline = QueryPipeline(chain=chain, verbose=verbose)
...
def create_query_engine(index, mode, **kwargs):
    ...
    if mode == "chat":
        ...
        raise ValueError(
            f"Invalid mode: {mode}. Pick one of 'chat', 'query', or 'retrieve'."
        )
...
def sample_documents(documents_by_author, num_samples=10):
    ...
    for author, docs in documents_by_author.items():
        ...
        if len(valid_docs) >= num_samples:
            ...
            print(
                f"Author {author} does not have enough valid documents to sample {num_samples}."
            )
...
def run_generations_on_eval_set(eval_dataset, col_name, query_pipeline, time_out=True):
    ...
    for i, row in tqdm(
        enumerate(eval_dataset), total=len(eval_dataset), desc="Generating answers"
    ):
